...
#here, it's problems so 1:24 each 15 times
X_temp$instance <- rep(1:24, each=15)
...
for (fold in folds){
  ...
  best_coco_multiple_iter <- best_coco_multiple[test_rows]
  ...
  rf_results = test_ml(rf_functional, test, test, "rf", train_data, multilabel = T, classes = best_coco_multiple_iter)
